...
async function testElevenLabsSTTWithWAV() {
  console.log('Testing ElevenLabs Speech-to-Text API with 16kHz mono WAV...');
  ...
  try {
    ...
    const testAudioPath = path.join(__dirname, 'test-audio-16k-mono.wav');
    ...
    console.log('Format: 16kHz mono WAV (PCM)');
    ...
    try {
      ...
      console.log('ElevenLabs Speech-to-Text API successfully processed 16kHz mono WAV!');
    } catch (sttError) {
      ...
      if (sttError.message && (sttError.message.includes('format') || sttError.message.includes('unsupported'))) {
        console.error('The 16kHz mono WAV format might not be supported by ElevenLabs API.');
      }
    }
  } catch (error) {
