...
export default function LoginPage() {
  ...
  }, [user, processTokensInUrl, redirectPath, authComplete, nextPath, supabase.auth, isInRedirectLoop]);
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center max-w-md p-6 bg-red-50 rounded-lg border border-red-200">
          ...
          <p className="mb-4 text-gray-800">
            We've detected a redirect loop in the authentication process. This has been temporarily fixed.
          </p>
          <p className="mb-6 text-gray-600">
            Please try signing in again using the form below.
          </p>
          <button
            onClick={() => {
  ...
  const handleSubmit = async (email: string, password: string, isSignUp: boolean) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Check if the user needs to verify their email using Supabase directly
        ...
        // Redirect to the original path if available, otherwise to dashboard
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Note: Redirection happens automatically after successful OAuth
    } catch (error) {
